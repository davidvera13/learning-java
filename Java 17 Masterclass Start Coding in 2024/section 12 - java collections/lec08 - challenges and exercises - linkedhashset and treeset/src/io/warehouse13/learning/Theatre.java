...
public class Theatre {
    ...
    public String reserveSeat(char row, int seat) {
        ...
        // will return requested seat if exists or null
        // work also with floor: we expect exact match
    ...
    public Set<Seat> reserveSeats(
            int count,
            char minRow,
            char maxRow,
            int minSeat,
            int maxSeat) {
        // take letter of last row
        ...
        Set<Seat> reservations = null;
        if(selected != null) {
            selected.forEach(seat -> seat.reserved = true);
            reservations = new TreeSet<>(selected);
        }
        return reservations;
    }
