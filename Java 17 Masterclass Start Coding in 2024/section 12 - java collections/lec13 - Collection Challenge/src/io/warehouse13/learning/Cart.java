...
public class Cart {
    ...
    public void addItem(InventoryItem item, int qty) {
        ...
        if(!item.reserveItem(qty)) {
            System.out.println("oops something went wront, could not add item");
        }
    }
    ...
    public void removeItem(InventoryItem item, int qty) {
        ...
        if(current <= qty) {
            ...
            products.merge(item.getProduct().sku(), qty, (prevQty, newQty) -> prevQty - newQty);
            System.out.printf("%d [%s] removed from basket %n", qty, item.getProduct().name());
        }
    ...
    public void printSalesSlip(Map<String, InventoryItem> inventory) {
        double total = 0d;
        System.out.println("********************************************");
        System.out.println("Thanks for your sale: ");
