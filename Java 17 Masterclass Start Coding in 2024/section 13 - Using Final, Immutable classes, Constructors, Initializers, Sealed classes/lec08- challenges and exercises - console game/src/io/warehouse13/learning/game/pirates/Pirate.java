...
import io.warehouse13.learning.game.Player;
...
public class Pirate implements Player {
    ...
    private void adjustValue(String name, int adjustedValue) {
        gameData.compute(name, (k, v) -> v += adjustedValue);
    }

    private void adjustHealth(int adjustedValue) {
        int health = value("health");
        health += adjustedValue;
        //health = (health < 0) ? 0: (health > 100) ? 100 : health:
        health = (health < 0) ? 0: Math.min(health, 100);
        adjustValue("health", health);
    }
